...
function generate_records($contacts_json,$current,$rapidpro) {
    ...
    foreach ($contacts_json as $contact) {
        ...
        //check if there is an Airtel number and ignore the rest (starts with 076 or 078 or 079)
        $airtel=false;
        $non_airtel=array();
        foreach ($record["urns"] as $id=>$rec) {
        		$rec=str_replace("+232","",$rec);
				if((substr($rec,0,7)==="tel:076") or (substr($rec,0,6)==="tel:76") or (substr($rec,0,7)==="tel:078") or (substr($rec,0,6)==="tel:78") or (substr($rec,0,7)==="tel:079") or (substr($rec,0,6)==="tel:79"))
					$airtel=true;
				else
					$non_airtel[]=$id;
        }
        if($airtel) {
        		foreach ($non_airtel as $index) {
					unset($record["urns"][$index]);
        		}
        		array_values($record["urns"]);
        }
